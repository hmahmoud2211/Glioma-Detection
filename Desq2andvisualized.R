...
path <- paste(urld, "acc=GSE48865", "file=GSE48865_raw_counts_GRCh38.p13_NCBI.tsv.gz", sep="&");
...
# sample selection
gsms <- paste0("11111X1111111X1X1011X0X1010X0X0010X1X100X1XXX110X1",
               "11X10000100010X010X111101X1011X11X0XXX11X1X0X1X10X",
               "0111X11XX101XX1X11X1X1XX1X11XX1111XX111111XXXX1X10",
               "XX010XXXX11XX0XXXX000XX0X00XX0000XXXX00X0XXXXXX001",
               "XX000X0X0XX1X01X0XXXXXXXX10XXX1000100X100X01XX0XXX",
               "XX1X00010X00X0X01XX1XXX0")
...
gs <- factor(sml)
groups <- make.names(c("Grade_3","Grade_2"))
levels(gs) <- groups
...
plotDispEsts(ds, main="GSE48865 Dispersion Estimates")

# create histogram plot of p-values
hist(r$padj, breaks=seq(0, 1, length = 21), col = "grey", border = "white", 
     xlab = "", ylab = "", main = "GSE48865 Frequencies of padj-values")
...
par(mar=c(7,4,2,1))
boxplot(dat[,ord], boxwex=0.6, notch=T, main="GSE48865", ylab="lg(norm.counts)", outline=F, las=2, col=gs[ord])
